...
output:
  ...
  fig_caption: yes
bibleography: My_Collection.bib
---
...
Recognition. It has been used in practice in the method M3C [-@john2020]
...
resulting matrix as a basis of a dissimilarity matrix for clustering. Senbabaofgly [ -@șenbabaouglu2014] 
...
success_wrap = success_wrap + facet_grid(mc_type ~ index) + 
  ...
  scale_fill_manual(name = 'Number/nof Correct/nValue of K/nIdentified',values = grad_cols[c(1,2,5,10,15,20,25)])
...
sums_wrap = sums_wrap + facet_grid(mc_type ~ index) + 
  ...
  scale_fill_manual(name = 'Number/nof Total/nValue of K/nIdentified', values = grad_cols)
...
pval_wrap = pval_wrap + facet_grid(mc_type ~ index) + 
